...
## Round 10 Quest/Chunk Edit Flow Unification (2026-02-28)
...
5. 청크 수정은 `prompt` 기반 입력을 제거하고 팝업 폼(미션 제목/소요시간)으로 전환한다.
6. 대기 목록 펼침 상태에서 청크를 드래그 앤 드롭으로 재정렬할 수 있게 한다.
7. 펼친 청크 행의 표시를 `미션 제목` + `소요시간(분)`으로 단순화하고 상태/중복 시간 표기를 제거한다.
