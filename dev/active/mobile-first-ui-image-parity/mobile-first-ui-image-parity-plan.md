...
이번 라운드는 단순 스타일 보정이 아니라 홈 카드 정보 구조 재정의(현재 퀘스트/다음 미션/편집/삭제)와 미션 아이콘 데이터 모델 확장을 포함한다.
...
5. 몬스터를 에너지 링서클 내부에 배치하고, 링은 타이머/미션 진행률에 따라 감소한다.
6. 미션 완료 시 링서클 에너지가 즉시 감소하도록 남은 시간 합산 로직을 반영한다.
...
## Round 9 Mobile Layout Stability (2026-02-28)
1. 모바일(`max-width: 420px`)에서도 퀘스트 생성 모달의 시간 카드 3열 배치를 유지한다.
2. 모바일에서도 현재 퀘스트 카드의 `예상소요시간/마감시간/마감까지` 3열 배치를 유지한다.
3. 대기중 퀘스트 메타 아이콘/텍스트 가독성을 강화한다.
4. 대기중 퀘스트 메타는 아이콘 1개가 2줄 텍스트를 커버하는 시각 구조로 맞춘다.

## Round 10 Quest/Mission Edit Flow Unification (2026-02-28)
1. 대기중 퀘스트의 별도 `퀘스트 수정` 버튼을 제거한다.
2. 대기중 퀘스트 헤더 우측에 메뉴 아이콘을 두고 `수정/삭제` 액션을 제공한다.
3. 퀘스트 수정은 기존 퀘스트 추가 모달을 재사용하며 `추가(create)`/`편집(edit)` 모드를 분리한다.
4. 편집 모드에서는 모달 타이틀/확인 버튼 문구를 `퀘스트 수정`으로 고정한다.
5. 미션 수정은 `prompt` 기반 입력을 제거하고 팝업 폼(미션 제목/소요시간)으로 전환한다.
6. 대기 목록 펼침 상태에서 미션를 드래그 앤 드롭으로 재정렬할 수 있게 한다.
7. 펼친 미션 행의 표시를 `미션 제목` + `소요시간(분)`으로 단순화하고 상태/중복 시간 표기를 제거한다.
...
3. 미션 아이콘 모델:
- `Mission.iconKey?: MissionIconKey`를 도입하고 생성 경로(로컬/AI/템플릿/리미션)에서 유실 없이 매핑
4. 홈 카드 액션 재사용:
- 기존 `handleEditMission`, `handleDeleteMission`, `handleEditTaskTotalMinutes`를 홈 뷰에 연결
5. 헤더 정합 방식:
...
- `Mission.iconKey` 타입/스토리지/청킹 매핑 추가
- 기본 아이콘 선택 함수 추가
