...
2. `P0`: 섹션 3/7 (`ui.png`) 입력 클러스터/세부 디테일 정합
3. `P1`: 타이포/아이콘/레이더 폴리곤 미세 보정
4. `P2`: 장식 요소/모션/햅틱 가이드 후순위 보정

## Implementation Phases
### Phase 1: 모바일 셸 재구성
- 렌더 순서(헤더 -> 상태 -> 입력 -> 퀘스트 -> 탭바) 고정
- safe-area 패딩 + 하단 탭바 고정 + 본문 스크롤 분리
...
- 입력/마이크/`AI가 쪼개기`를 엄지 영역 2탭 이내 흐름으로 유지
...
- 대기중 리스트 메타(시작/마감/소요) 가독성 및 인터랙션 충돌 방지
- 추가/편집 모달의 시간 카드/프리셋/생성 CTA 흐름 고정
...
- 핵심 경로(`입력 -> AI 쪼개기 -> 시작 -> 일시정지 -> 완료`)가 회귀 없이 동작한다.
