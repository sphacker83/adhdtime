# Task Manual Mission Side Plan - Tasks

Last Updated: 2026-02-28

## Phase Checklist
- [x] Phase 0: 사이드 플랜 문서 생성 및 초기 기준 확정
- [x] Phase 1: 수동 미션 생성 + 입력창 STT/생성 버튼 재배치
- [x] Phase 2: 홈 "오늘의 퀘스트" 폴딩/언폴딩
- [x] Phase 3: 실행 중 미션 시간 조정 ±5 추가
- [x] Phase 4: 미션 생성 입력 3필드 자동 계산
...
## Phase 1: 수동 미션 생성 + 입력창 STT/생성 버튼 재배치
- [x] Feature 1: 선택 과업 대상 수동 미션 생성 흐름 구현
- [x] Feature 2: STT 버튼을 입력창 내부 우측 마이크 아이콘으로 이동
- [x] Feature 3: 기존 STT 위치를 `미션 생성` 버튼으로 교체하고 Feature 1에 연결
...
- [x] 언폴딩 시 과업 내 미션 목록 표시(행동/상태/시간)
- [x] AC 점검: 폴딩 상태 전환, 빈 상태, 탭 이동 후 정책 일관성
- [x] 테스트 시나리오 실행: 과업 0/1/N개, 미션 상태 변경 직후 갱신

## Phase 3: 실행 중 미션 시간 조정 ±5 추가
- [x] Feature 5: `-5/-1/+1/+5` 버튼 배치
- [x] 기존 최소/최대/예산 제한 및 이벤트 로깅(`mission_time_adjusted`) 유지
- [x] AC 점검: 경계값/예산 초과 차단, 피드백 문구 일관성
- [x] 테스트 시나리오 실행: 최소/최대 경계, 예산 한계, 남은 시간 환산

## Phase 4: 미션 생성 입력 3필드 자동 계산
