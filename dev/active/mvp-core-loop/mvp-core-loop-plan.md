...
- PRD의 Chunk/TimerSession/XP/5스탯/이벤트 택소노미 미구현
- 문서와 UI IA 불일치

## Target State
- 홈: 상태 카드 + 입력 바 + 오늘의 청크 + 하단 탭
- 하이브리드 청킹: 로컬 룰 우선, AI 폴백 어댑터
- 청크 상태머신: todo/running/paused/done/abandoned/archived
...
1. 타입 정의(Task/Chunk/TimerSession/Stats/Event)
2. 로컬 청킹 + AI 폴백 스텁
3. 청킹 결과 validator
4. 보상 계산 유틸
5. 이벤트 큐 유틸
...
1. 도메인 타입 정합화(`summary`, `parentChunkId`, `rescheduledFor`, 상태 확장)
...
- 재청킹 시 원본 청크 `archived`, 신생 청크 `parentChunkId` 연결이 보장된다.
- 재등록 시 대상 청크 `abandoned` + `rescheduledFor` 기록이 보장된다.
