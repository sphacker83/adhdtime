# Task Manual Mission Side Plan - Plan

Last Updated: 2026-02-28

## Executive Summary
홈/입력 플로우의 미션 조작성을 높이기 위해 별도 사이드 플랜 트랙을 분리한다. 이번 트랙은 코드 구현 전 문서 기준을 먼저 확정하고, 사용자 요청 6개를 `입력 UX 개선 -> 홈 가시성 개선 -> 시간 계산 자동화` 순서로 적용한다.

## Current State (2026-02-27 구현 반영)
- 과업 입력 카드에서 STT는 입력창 내부 우측 아이콘 버튼으로 동작하며, 기존 STT 상태/에러/미지원 메시지는 유지된다.
- 입력 카드 보조 액션은 `미션 생성` 버튼으로 연결되어 선택 과업 대상 수동 미션를 추가한다.
- 홈 탭 `오늘의 퀘스트`는 과업별 폴딩/언폴딩을 지원하며, "남은 N개"와 펼침 목록 모두 actionable(`todo/running/paused`) 기준으로 일치한다.
- 실행 중 미션 시간 조정은 `-5/-1/+1/+5`를 지원하고 기존 최소/최대/예산 제약을 그대로 적용한다.
- 미션 생성 입력 3필드는 3개 조합 자동 계산을 지원하며, `last edited field` 우선 정책과 입력 단계 즉시 검증(MIN/MAX, 시작<=마감)을 적용한다.
...
### Feature 1. 사용자 수동 미션 생성 가능 (Priority: P0)
대상: 선택된 과업에 사용자가 직접 `action + estMinutes` 미션를 추가할 수 있어야 한다.
...
Acceptance Criteria:
1. 활성 과업이 선택된 상태에서 수동 미션 생성 액션을 실행하면 해당 과업에 미션가 추가된다.
2. 생성된 미션는 기존 정렬 규칙(order)과 예산 규칙(totalMinutes 대비 mission 합계)을 위반하지 않는다.
3. 실행 잠금 상태(`running` 또는 `paused` 미션 존재)에서 금지해야 하는 편집/생성 조건이 있으면 동일 정책으로 차단된다.

Risks:
- 예산 초과/순서 충돌로 인해 미션 상태 불일치가 발생할 수 있다.
- 수동 생성과 AI 생성이 혼재될 때 이벤트 로그 메타 정의가 모호해질 수 있다.

Test Scenarios:
1. 정상 케이스: 활성 과업 선택 후 수동 미션 생성 -> 목록/예산/정렬 반영 확인.
2. 경계 케이스: 과업 예산 직전 상태에서 추가 생성 -> 차단 메시지 확인.
3. 상태 케이스: 실행 중 미션가 있는 상태에서 정책 위반 생성 시도 -> 차단 확인.

Completion Criteria:
- 수동 미션 생성 성공/실패 경로가 UI 피드백과 이벤트 로그에 일관되게 남는다.
...
### Feature 3. 기존 STT 버튼 위치에 "미션 생성" 버튼 배치 (선택 과업에 미션 추가) (Priority: P0)
대상: 기존 STT 버튼 자리를 `미션 생성` 액션으로 교체하고, 선택 과업 대상 수동 미션 생성 진입점으로 사용한다.
...
Acceptance Criteria:
1. 기존 STT 버튼 위치에 `미션 생성` 버튼이 노출된다.
2. `미션 생성` 버튼은 활성 과업이 없으면 비활성화되거나 생성 대상을 먼저 선택하도록 안내한다.
3. `미션 생성` 실행 시 Feature 1의 수동 미션 생성 로직으로 연결된다.
...
2. 버튼 클릭 시 대상 과업 ID와 생성된 미션의 taskId 일치 확인.
3. 기존 AI 생성 흐름과 충돌 없이 공존하는지 회귀 확인.

Completion Criteria:
- 사용자가 입력 카드에서 생성 액션을 통해 명시적으로 선택 과업에 미션를 추가할 수 있다.

### Feature 4. 홈 "오늘의 퀘스트" 폴딩/언폴딩으로 과업 내 미션 확인 (Priority: P1)
대상: 홈 탭 리스트에서 과업별 미션를 펼쳐 볼 수 있는 접기/펼치기 UI를 추가한다.
...
2. 언폴딩 시 해당 과업의 actionable 미션 목록(행동명/상태/남은 시간 또는 예정 시간)을 확인할 수 있다.
...
3. 미션 상태 변경(시작/완료) 직후 언폴딩 목록 갱신 확인.

Completion Criteria:
- 홈 탭에서 과업별 미션를 즉시 확인할 수 있어 태스크 전환 없이 현재 진행 문맥을 파악할 수 있다.

### Feature 5. 실행 중 미션 시간 조정에 ±5 추가 (Priority: P1)
...
Acceptance Criteria:
1. 실행 중 미션 영역에 `-5분`, `-1분`, `+1분`, `+5분` 액션이 표시된다.
2. 최소/최대 미션 시간, 과업 예산 초과 방지 규칙이 기존과 동일하게 적용된다.
3. 이벤트 로그(`mission_time_adjusted`)의 `deltaMinutes`에 ±5가 기록된다.
...
3. 남은 시간 비율 환산 로직(remainingSecondsByMission) 정확성 확인.

Completion Criteria:
- ±1과 ±5 모두 정책 위반 없이 동작하고 이벤트/피드백이 일관된다.

### Feature 6. 미션 생성 시 `totalMinutes/scheduledFor/dueAt` 3필드 중 2개 입력 시 1개 자동 계산 (Priority: P0)
대상: 미션 생성 입력에서 세 필드 관계를 수학적으로 연결해 입력 마찰을 줄인다.
