...
기준 문서: `docs/PRD.md` v3.3, `docs/USECASE.md` v2.1
...
### Epic G. 성장 2축 분리(XP vs 스탯 랭크)

1. G-01 스탯 랭크 도메인 모델 도입
- `F~SS+` 단계, 스탯별 진행도(1~100), `100` 도달 시 랭크업 규칙 정의
- `SS+` 이후 `Mastery` 누적 규칙 포함

2. G-02 캐릭터 랭크 계산기
- 5개 스탯 평균 점수 기반 캐릭터 랭크 산식 구현
- 캐릭터 아이콘 옆 랭크/다음 랭크 진행률 노출

3. G-03 보상 파이프라인 분리
- 미션 완료 시 XP/스탯 진행도 동시 지급
- XP(계정 성장)와 스탯(행동 역량)의 계산/표시 경로 분리

4. G-04 페이싱 튜너
- `3~5 퀘스트/일` 사용자가 특정 스탯 `S` 도달까지 약 1년이 되도록 계수 튜닝
- 월간 코호트 기반 보정 운영(과속/저속 자동 경고)

5. G-05 일일 보상 상한(Quest Daily Cap)
- 일일 보상 대상 퀘스트 수를 최대 5개로 제한(`AXP/SGP`)
- 6번째 이후 완료는 기록/KPI만 반영하고 보상은 미지급
- 로컬 타임존 기준 리셋 시각(04:00) 및 카운트 정책 구현

### Epic H. Co-op Room Event (단체 협동)

1. H-01 방 생성/참여/종료 도메인 모델
- Room, Member, RoomMission, TeamGoal 구조 정의

2. H-02 방 진행률 및 개인 기여도 집계
- 개인 완료분을 방 목표에 반영하는 집계 로직
- 협동 우선 UI(팀 진행률 중심) 설계

3. H-03 이벤트 보상/뱃지 시스템
- 방 목표 달성 시 공동 보상 지급
- 개인 보상(XP/스탯) + 팀 보상(뱃지/테마) 이중 보상 규칙

4. H-04 계측/안전/악용 방지
- 신규 이벤트(`room_created`, `room_joined`, `room_mission_completed`, `room_goal_completed`) 수집
- 다계정/반복 파밍 방지 정책 및 레이트 제한
