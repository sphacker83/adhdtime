...
이 문서는 엔진 구현은 건드리지 않고, 추천 품질을 결정하는 데이터셋을 Codex로 대량 **작성(직접 출력)**하고, **검증 명령을 실행**해 통과한 뒤, 패키징하기 위한 작업 지침입니다.
...
※ `data/validation_rules.json`은 데이터 작성/검증을 위한 엄격한 **단일 진실 기준(참조용)**이므로 AI 에이전트가 새로 만들지 않습니다. 이미 픽스된 파일을 읽고 무조건 따릅니다.
...
1.3 100% AI(LLM) 직접 작성(출력) 강제 (프로그램적 조립/치환 절대 금지)
	•	클러스터, 컨셉, 템플릿, 렉시콘(Variants) 등 모든 JSON 노드 레코드는 AI가 맥락과 페르소나를 이해하고 하나씩 직접 창작하여 작성해야 합니다.
	•	절대 코드(Python, Node.js 등)로 키워드의 경우의 수를 끼워 맞추거나 조립/치환하는 식의 기계적인 생성(데이터 뻥튀기)을 하지 마십시오. 프로그램적 조립/치환에 의존할 경우 앞뒤가 맞지 않는 기계적이고 엉뚱한 가비지 데이터가 양산됩니다.
...
2.3 인간 친화적(대화체) 문장 규칙
	...
	•	미션(action)은 “행동 지시문”으로 쓴다(존댓말 지시문 가능/권장).
...
Step F) 검증/정제(반복)
	•	`npm run -s dataset:validate` **명령을 실행**해 실패 항목을 수집 (검증은 “데이터 생성”이 아니라 “품질 게이트”다)
...
4.1 작성 → 검증(명령 실행) → 재작성 루프
	1.	클러스터/컨셉/매핑을 먼저 작성(확정)
	2.	템플릿을 클러스터 단위로 **묶어서 직접 창작**해 추가한다(“묶음(작성 단위)”=코드로 자동 조립/치환하는 뜻이 아니라, 사람이 읽을 수 있는 문장을 여러 개 작성하는 단위)
	3.	lexicon을 컨셉 단위로 **묶어서 직접 창작**해 variants를 확장한다
	4.	검증 명령 실행(`npm run -s dataset:validate`)
	5.	실패 목록만 AI 모델이 직접 실패 원인을 인지하고 사람의 관점에서 자연스럽게 재작성 (프로그램적 기계 치환/조립 금지)
	6.	반복

4.2 작성 단위(권장)
	•	클러스터 단위(직접 창작): 클러스터 1개당 템플릿 20개 작성(각각 문장/미션이 실제로 다르게)
	•	컨셉 단위(직접 창작): 컨셉 1개당 variants 50~150개 작성
...
8) 최소 운영 루프(데이터 성장)
	...
	3.	해당 상황에 맞는 템플릿 3~10개 추가 작성
	4.	validate 통과 → 배포

⸻

9) Codex 작업 지시(데이터셋 작성만)

스키마/무결성/금지어는 docs/dataset-schemas.md 및 data/validation_rules.json을 참조하라.

목표: 추천 엔진용 데이터셋을 작성/검증/패키징한다.
...
- data/validation_rules.json (읽기 전용, 단일 진실 기준)

※ 주의: `data/validation_rules.json`은 읽기 전용이며, 데이터셋 검증을 위한 **단일 진실 기준**으로, 반드시 이 파일을 먼저 읽고 규칙에 절대 복종해야 함.

규칙:
- [절대 엄수 금지] 코드(Python, Node.js 등)로 데이터(키워드/템플릿/렉시콘 등)를 프로그램적으로 치환/조립하여 양산하는 행위를 100% 금지합니다. 모든 JSON 레코드는 AI 모델이 현실 상황을 상상하며 건바이건으로 직접 텍스트를 출력(창작)해야 합니다.
...
- 검증은 `npm run -s dataset:validate` **명령을 실행**해 수행한다. (검증은 품질 게이트이며, 데이터 생성용이 아니다)
- **데이터 작성을 위해 자동 조립/치환용 코드를 새로 만들거나 수정하지 않는다.** (사용자가 명시적으로 요청한 경우만 예외)
- 실패한 템플릿은 삭제하지 말고 재작성 대상으로 목록화한다.

출력:
- 데이터셋 6파일을 완성본으로 준비(5개 직접 작성 + validation_rules는 읽기 전용 유지)
